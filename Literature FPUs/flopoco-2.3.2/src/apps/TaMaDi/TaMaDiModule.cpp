/*
  TaMaDiModule evaluation of worst cases for rounding functions without 
...
namespace flopoco{
	...
	Operator(target), wp(wp), d(d), iterations(iterations), wIntervalID(wIntervalID), n(n) 
	{
		...
		name <<"TaMaDiModule_wp"<<wp<<"_interations"<<iterations<<"_degree"<<d<<"_uid"<<getNewUId();
		setName(name.str()); 
		...
		addOutput  ("MainFIFOOutputWESignal");
