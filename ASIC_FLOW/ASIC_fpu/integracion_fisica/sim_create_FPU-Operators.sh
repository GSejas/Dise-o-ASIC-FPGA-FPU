...
do
    ...
    cp Testbench_all_operators.v
    for PRES in "${precision_list[@]}"
    do
    mkdir  $sim_syn_dir/$i/$PRES
    #SAIF SCRIPT FILES ARE CREATED
     cp Testbench_all_operators.v "$sim_syn_dir/$i/$PRES"
     cp ibm13rflpvt.v $sim_syn_dir/$i/$PRES
      SAIF_FILE "$sim_syn_dir/$i/$PRES" "$PRES ${i}_$PRES" "get_saif_$i$PRES.sh"

    #RUN SCRIPT FILES ARE CREATED FOR EACH PRECISION
      RUN_SIM_FILE "$sim_syn_dir/$i/$PRES" "$PRES ${i}_$PRES" "run_sim_$i$PRES.sh"
