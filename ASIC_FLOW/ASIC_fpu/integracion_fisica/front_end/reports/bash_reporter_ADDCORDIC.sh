...
MOD_FILENAME="FPU_Interface2"
multiplicador_lst=("DW_1STAGE" "RKOA_1STAGE" "RKOA_2STAGE" "KOA_1STAGE" "KOA_2STAGE")
max_clk_lst=(ASIC_fpu_syn_constraints_clk10.tcl ASIC_fpu_syn_constraints_clk20.tcl ASIC_fpu_syn_constraints_clk30.tcl ASIC_fpu_syn_constraints_clk40.tcl ASIC_fpu_syn_constraints_noclk.tcl)
  ...
  do
    for o in "${match_report_string_lst[@]}"
      do
            ...
            do
                  output_text=$(find SINGLE -name "$MOD_FILENAME\_$i\_$e\_syn_qor.txt" -exec grep -i "$o" {} \;)
                  #echo $output_text
                 ...
                 # echo ${e}
                  ...
                  echo "           $number" >> "${Txt_design_name[$z]}\_${e}.txt"
  ...
  do
    ...
    #   do
            #echo "Power" >> "${e}.txt"
             for i in "${max_clk_lst[@]}"
            do
                  output_text=$(find SINGLE  -name "$MOD_FILENAME\_$i\_$e\_syn_power.txt" -exec grep -i "Total\ \ \ " {} \;)
                  #echo $output_text
                  number=$(echo $output_text  | grep -oP '[0-9]*.(\d+)(?!.*\d)' | sed -e "s/^/${dir_list4[$x]}    /")
                  echo $number
                 # echo ${e}
                  #echo "${dir_list4[$z]} $number"
                  echo "           $number" >> "Power_${e}.txt"
