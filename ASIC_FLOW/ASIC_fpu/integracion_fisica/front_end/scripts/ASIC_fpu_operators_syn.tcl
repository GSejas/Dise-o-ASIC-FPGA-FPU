...
set OP_LIST "CORDIC_Arch2 CORDIC_Arch3 FPU_Add_Subtract_Function FPU_PIPELINED_FPADDSUB"
...
while {$x < 2} {
    foreach OP_LINE $OP_LIST_LINE {
		...
		link
		 ...
		 ./db/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn_unmapped.ddc
		...
		./db/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn.v

		#Generar los reportes

		report_power -analysis_effort high > reports/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn_power.txt
		report_area >   reports/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn_area.txt
		report_cell >   reports/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn_cell.txt
		report_qor >    reports/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn_qor.txt
		report_timing > reports/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn_timing.txt
		report_port >   reports/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn_port.txt

		#Escribir el archivo *.ddc (base de datos sintetizada)
		write -hierarchy -format ddc -output \
		./db/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn_mapped.ddc
		...
		write_sdc ./db/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn.sdc
		write_sdf ./db/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn.sdf
		write_sdf ../simulacion_logica_sintesis/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn.sdf

		##LE AGREGAMOS CON UN COMANDO DE BASH EL SDF CORRESPONDIENTE PARA LA SIMULACION
		set string_replace "sed -i \"s/endmodule/initial\ \\\$sdf\_annotate\(\\\"$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn.sdf\\\"\)\\\\; \\n endmodule/g\" db/$PRECISION($x)/$OP_LINE\_$CONTRAINTS_FILE_NAME\_syn.v"
